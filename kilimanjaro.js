...
const kali = readFileSync('./kilimanjaro.jpg'); // Using readFileSync directly
import { Client } from "genius-lyrics"; 
import { isUrl, processTime } from "./lib/kilimanjarofunc.js";
import { TelegraPh, UploadFileUgu } from './lib/kilimanjaroupload.js';
import { smsg, formatp, tanggal, formatDate, getTime, sleep, clockString, fetchJson, getBuffer, jsonformat, generateProfilePicture, parseMention, getRandom, MediaFire } from './lib/kilimanjarofunc.js';
...
export default async function kilimanjaro(client, m, chatUpdate, store) {
...
var msgKilimanjaro = m.message.extendedTextMessage?.contextInfo?.quotedMessage;
...
const pict = await fs.readFileSync('./kilimanjaro.jpg');
const mode = process.env.MODE || 'PUBLIC';
const author = process.env.STICKER_AUTHOR || 'venocybertech';
...
const antilink = process.env.ANTILINK || 'TRUE';
    ...
    const Owner = DevKilimanjaro.map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender)
...
const audiovn = "./alive.mp3";
    const dooc = {
        ...
        fileName: "Kilimanjaro",

        contextInfo: {
          mentionedJid: [m.sender],
          externalAdReply: {
          title: "Hi human, I am Alive",
          body: "KILIMANJARO BOT",
          thumbnailUrl: "https://telegra.ph/file/f1743cfafa7bd3c4c4d11.jpg",
          sourceUrl: '',
          mediaType: 1,
...
let dreadrecordin = ['recording','composing']
        ...
        let kilimanjarorecordinfinal = kilimanjarorecordin[Math.floor(Math.random() * kilimanjarorecordin.length)]

        client.sendPresenceUpdate(kilimanjarorecordinfinal, from)
}
...
return;
    ...
    if (cmd && !m.isGroup) {
      console.log(chalk.black(chalk.bgWhite("[ KILIMANJARO-MD ]")), color(argsLog, "turquoise"), chalk.magenta("From"), chalk.green(pushname), chalk.yellow(`[ ${m.sender.replace("@s.whatsapp.net", "")} ]`));
        ...
        default: {
          if (cmd && budy.toLowerCase() != undefined) {
            ...
            if (argsLog || (cmd && !m.isGroup)) {
              ...
              console.log(chalk.black(chalk.bgRed("[ ERROR ]")), color("command", "turquoise"), color(`${prefix}${command}`, "turquoise"), color("Kilimanjaro", "turquoise"));
